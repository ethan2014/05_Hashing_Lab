...
HashTable<Key,T>::HashTable()
{
	...
	numRemoved = 0;
	primesCount = 0;
	backingArraySize = hashPrimes[primesCount];
...
void HashTable<Key,T>::grow()
{
	...
	unsigned long int new_size = hashPrimes[primesCount];
	unsigned long int old_size = backingArraySize;
	...
	for (int i = 0; i < old_size; i++) {
		if (!backingArray[i].isNull && !backingArray[i].isDel) {
			...
			new_arr[h].k = backingArray[i].k;
			new_arr[h].x = backingArray[i].x;
			new_arr[h].isNull = false;
